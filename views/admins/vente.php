...
$nmaison_achat = $db_Admins->numberocur("maison_achat")->count;
...
<div id="allbody">
  ...
  <h1 class="h3 mb-0 text-gray-800">
    <span class="h2 text-info font-weight-bolder">
      <?php if($nmaison_achat > 1){ echo $nmaison_achat; } else { echo $nmaison_achat; } ?>
    </span> Maison(s) ou Appartement(s) à vendre</h1> 
    <form id="search_p" class="h4 text-info font-weight-bolder form-inline" method="post" style="min-width:45%;">
      <?php if($nmaison_achat != 0) {  ?> 
        <input type="search" class="form-control mr-sm-2 w-100" name="rech_vente" id="rech_vente" table="maison_achat" placeholder="Recherche ...">
      <?php } ?>   
    </form>
  </div>
          ...
          <tbody id="rest_recherche">
            <?php if($nmaison_achat == 0) {  ?> 
              <tr>
                  <td colspan="7">
                      <div class="card mb-3">
                          <img src="/storages/imgs/img/small_pic3.jpg" class="card-img-top " alt="..." style="height:600px">
                      </div>
                  </td>
              </tr>
            <?php } else { ?> 
            <?php $query_projet = $db_Immo->ventes(); $result = ''; foreach ($query_projet as $fetch) { ?>
              <tr>
                ...
                <td style="min-width: 15.2rem; text-align:right;"> 
                  ...
                  <a href="/nadmin/connect_admin/upload/<?=$fetch->id_article?>/vente"> <i class="btn btn-default fa fa-upload text-success" style="cursor: pointer"></i> </a>

                  <a href="/nadmin/connect_admin/galerie/<?=$fetch->id_article?>/vente"> <i class="btn btn-default fa fa-image" style="cursor: pointer; color:#884D7A"></i> </a>
                ...
                <td colspan="7">
                  ...
                  <div class="collapse" id="collapse_trash<?=$fetch->id_article?>">
                    <div class="card mb-3 border border-danger rounded p-3">
                      ...
                      <div class="row text-center">
                        ...
                        <div class="col-sm-6 mb-3">
                          <button id_article="<?=$fetch->id_article?>" class="btn btn-danger oui_trash">Oui</button>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="collapse" id="collapse_edit<?=$fetch->id_article?>">
                    ...
                    <div class="text-right">
                      <button data-toggle="collapse" data-target="#collapse_edit<?=$fetch->id_article?>" aria-expanded="false" aria-controls="collapse_edit<?=$fetch->id_article?>" aria-hidden="true" id_article="<?=$fetch->id_article?>" class="btn btn-outline-primary font-weight-bold oui_edit colashow">Modifier <i class="fa fa-edit"></i></button>
                    </div>
                  </div>
